...
func ParseId(idStr string) (Id, *MethodLevelError) {
	if !regexp.MustCompile("^[A-Za-z0-9-_]{1,255}?$").MatchString(idStr) {
		return Id(""), NewMethodLevelErrorInvalidArguments(fmt.Sprintf("invalid id %s", idStr))
	}
	return Id(idStr), nil
}
...
func (id *Id) UnmarshalJSON(b []byte) error {
	...
	if idStr == "" {
		return NewMethodLevelErrorInvalidArguments("accountId cannot be empty")
	}
...
func ParseIds(idStrs []string) (result []Id, failedId string, mErr *MethodLevelError) {
...
func ParseUint(i int64) (Uint, *MethodLevelError) {
	if i < 0 || float64(i) > (math.Pow(2, 53)-1) {
		return Uint(0), NewMethodLevelErrorInvalidArguments(fmt.Sprintf("uint out of range"))
	}
