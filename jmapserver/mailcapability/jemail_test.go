...
`
			...
			testutils.AssertNotNil(t, to[0].Name)
			...
			testutils.RequireNoError(t, mErr)
			if testutils.AssertNotNil(t, sendAt) {
				testutils.AssertEqual(t, eSendAt, time.Time(*sendAt).String())
			}
			...
			if ct, ok := ctIface.(string); !ok {
				...
				testutils.AssertEqual(t, eContentType, ct)
			}

			eReceived := msg.Received
			testutils.AssertEqual(t, eReceived.String(), time.Time(jem.ReceivedAt()).String())

			//this an email with no msg body so preview is empty
			ePreview := ""
			preview, mErr := jem.Preview()
			testutils.RequireNoError(t, mErr)
			testutils.AssertEqual(t, ePreview, preview)

			bs, mErr := jem.BodyStructure(nil)
			testutils.RequireNoError(t, mErr)

			if testutils.AssertNotNil(t, bs.Type) {
				testutils.AssertEqual(t, "text/plain", bs.Type.String())
			}
			if testutils.AssertNotNil(t, bs.Language) {
				//NB: | is an arbitrary token to stringify a string slice to make it comparable
				testutils.AssertEqual(t, strings.Join([]string{"en-US"}, "|"), strings.Join(bs.Language, "|"))
			}
			if testutils.AssertNotNil(t, bs.CharSet) {
				//NB: | is an arbitrary token to stringify a string slice to make it comparable
				testutils.AssertEqual(t, "UTF-8", *bs.CharSet)
			}

			jPart, mErr := jem.JPart()
			testutils.RequireNoError(t, mErr)
			ty := jPart.Type()
			testutils.AssertEqual(t, "text/plain", ty.String())

			testutils.AssertEqual(t, "0", jPart.ID())
		})
...
>`
			...
			testutils.AssertEqual(t, "5a51ce56-387a-1b2e-26bf-133f93c918c1@km42.nl", inReplyTo[0])
			...
			testutils.AssertEqual(t, "5a51ce56-387a-1b2e-26bf-133f93c918c1@km42.nl", references[0])
			...
			if body, ok := bv["0"]; !ok {
				...
				testutils.AssertEqual(t, "need a mail for testing\r\n\r\nOn 18-07-2023 17:59, me wrote:\r\n>", body.Value)
			}
			...
			if body, ok := bv2["0"]; !ok {
				...
				testutils.AssertEqual(t, "need", body.Value)
				testutils.AssertTrue(t, body.IsTruncated)
			}
			...
			if testutils.AssertNotNil(t, bs.PartId) {
				testutils.AssertEqual(t, "0", *bs.PartId)
			}

			if testutils.AssertNotNil(t, bs.BlobId) {
				testutils.AssertEqual(t, "1-0", string(*bs.BlobId))
			}
			...
			testutils.AssertNil(t, bs.Disposition)
			testutils.AssertNil(t, bs.Cid)

			testutils.AssertEqual(t, "en-US", strings.Join(bs.Language, ","))

			testutils.AssertNil(t, bs.Location)
...
--------------Z8pBLNP8kO35FOYVOKN5cUf4--`
			...
			testutils.AssertEqual(t, 2, len(bodyStructure.SubParts))

			if testutils.AssertNotNil(t, bodyStructure.SubParts[0].Type) {
				testutils.AssertEqual(t, "text/plain", bodyStructure.SubParts[0].Type.String())
			}

			if testutils.AssertNotNil(t, bodyStructure.SubParts[1].Type) {
				testutils.AssertEqual(t, "text/html", bodyStructure.SubParts[1].Type.String())
			}
...
`
			...
			testutils.AssertEqual(t, 2, len(bs.SubParts))

			testutils.AssertEqual(t, 2, len(bs.SubParts[1].SubParts))

			imgPart := bs.SubParts[1].SubParts[1]
			testutils.AssertEqual(t, "image/png", imgPart.Type.String())
			testutils.AssertEqual(t, "part1.Nj2N9maO.uVlYYEhk@km42.nl", *imgPart.Cid)
			testutils.AssertEqual(t, "kOp2KOEom97WsgRN.png", *imgPart.Name)
			testutils.AssertEqual(t, "2", *imgPart.PartId)

			bv, mErr := jem.BodyValues(false, true, false, nil)
			testutils.RequireNoError(t, mErr)
			testutils.AssertEqual(t, 2, len(bv))
			htmlBodyValue, ok := bv["1"]
			testutils.AssertTrue(t, ok)
			testutils.AssertEqual(t, "<!DOCTYPE html>\r\n<html>\r\n  <head>\r\n\r\n    <meta http-equiv=\"content-type\" content=\"text/html; charset=UTF-8\">\r\n  </head>\r\n  <body>\r\n    <p>My first image <img src=\"cid:part1.Nj2N9maO.uVlYYEhk@km42.nl\"\r\n        alt=\"\"></p>\r\n  </body>\r\n</html>", htmlBodyValue.Value)
			...
			testutils.AssertEqual(t, 2, len(jPart.JParts[1].JParts))
			testutils.AssertEqual(t, "", jPart.JParts[1].ID())
			testutils.AssertEqual(t, "1", jPart.JParts[1].JParts[0].ID())
			testutils.AssertEqual(t, "2", jPart.JParts[1].JParts[1].ID())
		})
...
--Apple-Mail-C592FF4B-B8D0-46D3-BEBB-B8200C3105AA--`
			...
			testutils.AssertEqual(t, 3, len(bs.SubParts))

			textBodyParts, merr := jem.TextBody(defaultEmailBodyProperties)
			testutils.RequireNoError(t, merr)
			testutils.AssertEqual(t, 3, len(textBodyParts))
			testutils.AssertEqual(t, "0", *textBodyParts[0].PartId)
			testutils.AssertEqual(t, "1", *textBodyParts[1].PartId)
			testutils.AssertEqual(t, "2", *textBodyParts[2].PartId)

			hasAttachment, merr := jem.HasAttachment()
			testutils.RequireNoError(t, merr)
			testutils.AssertTrue(t, !hasAttachment)
		})
...
`
			...
			testutils.AssertEqual(t, "Jmap", *sender[0].Name)
			...
			if testutils.AssertNotNil(t, charSet) {
				testutils.AssertEqual(t, "utf-8", *charSet)
			}
		...
		t.Run("Mail to JEmail.  html body part with mixed", func(t *testing.T) {
			...
			testutils.AssertEqual(t, 2, len(jPart.JParts))
			testutils.AssertEqual(t, "text/plain", jPart.JParts[1].Type().String())
			testutils.AssertEqual(t, "inline", *(jPart.JParts[1].Disposition()))
			...
			testutils.AssertEqual(t, `text/plain; charset="us-ascii"`, bodyPart.BespokeProperties[bespokeProp].(string))
			...
			testutils.AssertEqual(t, myMap[bespokeProp].(string), `text/plain; charset="us-ascii"`)
...
func TestEmailChanges(t *testing.T) {
	...
	testutils.RequireNoError(t, db.Insert(context.Background(), &store.Mailbox{
		Name: "mailboxname",
		ID:   1,
	}))
	...
	testutils.RequireNoError(t, createMsg(db, 1, 100, 0, false))
	...
	testutils.AssertEqual(t, "102", newState)
	testutils.AssertEqual(t, false, hasMore)
	testutils.AssertEqual(t, 1, len(created))
	testutils.AssertEqual(t, 1, created[0].Int64x())
	testutils.AssertEqual(t, 1, len(updated))
	testutils.AssertEqual(t, 2, updated[0].Int64x())
	testutils.AssertEqual(t, 1, len(destroyed))
	testutils.AssertEqual(t, 3, destroyed[0].Int64x())
	...
	testutils.AssertEqual(t, "102", newState)
	testutils.AssertEqual(t, false, hasMore)
	testutils.AssertEqual(t, 0, len(created))
	testutils.AssertEqual(t, 0, len(updated))
	testutils.AssertEqual(t, 0, len(destroyed))
	testutils.AssertEqual(t, nil, mErr)
	...
	testutils.AssertEqual(t, 1, retAccountID.Int64x())
	testutils.AssertEqual(t, "99", oldState)
	testutils.AssertEqual(t, "101", newState)
	testutils.AssertEqual(t, true, hasMore)
	testutils.AssertEqual(t, 1, len(created))
	testutils.AssertEqual(t, 1, len(updated))
	testutils.AssertEqual(t, 0, len(destroyed))
	testutils.AssertEqual(t, nil, mErr)
...
func TestChangeResultBuilder(t *testing.T) {
	...
	t.Run("test1", func(t *testing.T) {
		...
		testutils.AssertEqual(t, "102", state)
		testutils.AssertEqual(t, 1, len(created))
		testutils.AssertEqual(t, 1, len(destroyed))
		testutils.AssertEqual(t, 1, len(updated))
		...
		testutils.AssertEqual(t, 1, len(destroyed))
		testutils.AssertEqual(t, 0, len(updated))
		testutils.AssertEqual(t, "101", state)
	})
}
