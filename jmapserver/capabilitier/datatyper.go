...
type Getter interface {
	...
	Get(ctx context.Context, jaccount JAccounter, accountId basetypes.Id, ids []basetypes.Id, properties []string, customParams any) (retAccountId basetypes.Id, state string, list []any, notFound []basetypes.Id, mErr *mlevelerrors.MethodLevelError)
	CustomGetRequestParams() any
}

type Changeser interface {
	//https://datatracker.ietf.org/doc/html/rfc8620#section-5.2
	Changes(ctx context.Context, jaccount JAccounter, accountId basetypes.Id, sinceState string, maxChanges *basetypes.Uint) (retAccountId basetypes.Id, oldState, newState string, hasMoreChanges bool, created, updated, destroyed []basetypes.Id, mErr *mlevelerrors.MethodLevelError)
}

type Setter interface {
	//https://datatracker.ietf.org/doc/html/rfc8620#section-5.3
	Set(ctx context.Context, jaccount JAccounter, accountId basetypes.Id, ifInState *string, create map[basetypes.Id]any, update map[basetypes.Id]basetypes.PatchObject, destroy []basetypes.Id) (retAccountId basetypes.Id, oldState *string, newState string, created, updated, destroyed map[basetypes.Id]any, notCreated, notUpdated, notDestroyed map[basetypes.Id]mlevelerrors.SetError, mErr *mlevelerrors.MethodLevelError)
}
